...
<body>
	...
	<table id="dg" class="easyui-datagrid" title="账目管理页面" style="nowrap:false;width:1100px;height:600px"  toolbar="#toolbar"
			data-options="remoteSort:false,loadMsg:'数据加载中，请稍后...',singleSelect:true,nowrap:false,url:'../shoppingOrder/getData',rownumbers:true,pagination:true,pageSize:20">
        <thead>
            <tr>
                <th data-options="field:'customerId',sortable:true,width:100,align:'center'">用户ID</th>
                <th data-options="field:'customer',sortable:true,width:130,align:'center'" formatter="formatCustomer">客户名称</th>
                <th data-options="field:'shop',sortable:true,width:130,align:'center'" formatter="formatCompany">商店名称</th>
                <th data-options="field:'serialNumber',sortable:true,nowrap:false,width:150,align:'center'">流水号</th>
				<th data-options="field:'price',sortable:true,nowrap:false,width:100,align:'center'">支付金额</th>	
                <th data-options="field:'createDate',sortable:true,width:150,align:'center'">支付日期</th>
                <th data-options="field:'status',sortable:true,width:100,align:'center'" formatter="formatStatus">是否支付</th>
               <!--  <th data-options="field:'taxable',sortable:true,nowrap:false,width:80,align:'center'">纳税</th>	
				<th data-options="field:'shipping',sortable:true,nowrap:false,width:100,align:'center'">运费</th>	
				<th data-options="field:'price',sortable:true,nowrap:false,width:100,align:'center'">支付金额</th> -->
    ...
    <div id="toolbar" style="padding:5px;height:auto">	
		<div>
			...
			支付起始时间: <input class="Wdate" name="firstTime" type="text" onClick="WdatePicker()" style="width:102px" id="firstTime"/>
			支付结束时间: <input class="Wdate" name="lastTime" type="text" onClick="WdatePicker()" style="width:102px" id="lastTime"/>
			流水号: <input  name="querySerialNumber" type="text"  style="width:102px" id="querySerialNumber"/>
			
			是否支付: <select class="easyui-combobox" data-options="editable:false" name="selectStatus" id="selectStatus" style="width:82px">
		            <option value ="1">按已支付</option>
					<option value ="0">按未支付</option>
			    </select>	
...
<!-- 	<div id="dlg-buttons">
	...
	$(function(){
		...
		$.ajax({
		    type: "POST",
		    url: "<%=request.getContextPath() %>/shoppingOrder/getSumPriceData",
		    data:{},
		    dataType: "json",
		    success: function(data){
		 	   $("#dg").datagrid("getPanel").panel("setTitle","营业总额: "+data.sumPrice+" 元");
		    }
		});
    ...
    function datasearch(){
		...
		var querySerialNumber = $("#querySerialNumber").val();
		var selectStatus=$("#selectStatus").combobox("getValue"); 
    	...
    	$("#dg").datagrid({
    		onBeforeLoad:function(row){
    			...
    			row.serialNumber=querySerialNumber,
    			row.status=selectStatus
    	    }
    	});

...
<body>
	...
	<table id="dg" class="easyui-datagrid" title="留言板管理页面" style="nowrap:false;width:1100px;height:600px"  toolbar="#toolbar"
			data-options="remoteSort:false,loadMsg:'数据加载中，请稍后...',singleSelect:true,nowrap:false,url:'../shopMessage/getData',rownumbers:true,pagination:true,pageSize:20">
        <thead>
            <tr>
               <!--  <th data-options="field:'headerImage',sortable:true,formatter:showImage,width:160,align:'center'">头像</th>
                ...
                <th data-options="field:'id_',width:80,formatter:operationDetailed,align:'center'">操作</th>	
...
<!-- 	<div id="dlg-buttons">
	...
	function operationDetailed(value,row,index){
		var releaseScope="<input type=\"button\" onclick=\"javascript:deleteShopMessage('"+row.id_+"','"+row.shopId+"');\" value=\"删除\" /> ";
		return releaseScope;
	}
	function deleteShopMessage (id_ , shopId){
		
		$.messager.confirm('Confirm','请确认是否删除?',function(r){
			  if (r) {
				  $.ajax({
			          type: "post", 
	                  url: "<%=request.getContextPath()%>/shopMessage/delete",
	                  dataType: "json", 
	                  data: {
	                	  id_:id_,
	                	  shopId:shopId

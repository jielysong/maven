...
<body>
	...
	<table id="dg" class="easyui-datagrid" title="提现审批" style="width: 1100px; height: 600px" toolbar="#toolbar" nowrap="false"
		data-options="remoteSort:false,loadMsg:'数据加载中，请稍后...',singleSelect:true,url:'../withdraw/getData',rownumbers:true,pagination:true,pageSize:20">
		<thead>
			<tr>
				<!-- <th data-options="field:'withdrawId',sortable:true,width:160,align:'center'">提现Id</th> -->
				<th data-options="field:'serialNumber',sortable:true,width:160,align:'center'">流水号</th>
				<th data-options="field:'accountId',sortable:true,width:140,align:'center'">转账Id</th>
				<th data-options="field:'withdrawMoney',sortable:true,width:160,align:'center'">提现金额</th>
				<th data-options="field:'shopId',sortable:true,width:100,align:'center'">开始时间</th>
				<th data-options="field:'userId',sortable:true,width:100,align:'center'">结束时间</th>
				<th data-options="field:'customerId',sortable:true,width:100,align:'center'">创建人</th>
				<th data-options="field:'withdrawDate',sortable:true,width:150,align:'center'">申请时间</th>
 				<th data-options="field:'withdrawId',sortable:true,width:90,align:'center'" formatter="approve">审批</th>				
			</tr>
		</thead>
	</table>
	...
	<div id="becaon-dialog" class="easyui-dialog" style="width:300px;height:200px" closed="true" buttons="#becaon-dlg-buttons" data-options="iconCls:'icon-save',toolbar:'#becaon-toolbar',modal:true" >
	     <div id="beaconList" class="beaconList">
			<table id="dg_beacon" ></table>
		</div>
	     <input id="banner_withdrawId" type="hidden" />
	</div>
	
	<div id="becaon-dlg-buttons">
		<a href="javascript:void(0)" class="easyui-linkbutton" iconCls="icon-ok" onclick="updateWithdraw()">确定</a> 
		<a href="javascript:void(0)" class="easyui-linkbutton" iconCls="icon-cancel" onclick="cancle_dialog()">取消</a>
	</div>
	
	<div id="becaon-toolbar" style="padding:3px;height:auto">
		<div style="margin-bottom:3px">
     		<label>审批信息</label> 
			<input name="approveDesc" id="approveWithdraw" class="easyui-validatebox" data-options="validType:'maxLength[10]'">
    	</div>
	</div>
...
<script type="text/javascript">
	function approve(value,row){
	//		return "<a hrf='javascript:void(0);' onclick='showBeacon("+value+")'>设备详情</a>";
		return '<input type="button" value="审批" onclick="approveWithdraw(\''+row.withdrawId+'\')" ></input>';
	};
	
	function approveWithdraw(value){
			$('#becaon-dialog').dialog('open').dialog('setTitle', '提现审批');
			$('#dg_beacon').datagrid({
			        onBeforeLoad:function(data){
			        	$("#banner_withdrawId").val(value);
			        }
			    });
	}
	
	function updateWithdraw(){
		var approveDesc=$("#approveWithdraw").val();
		var withdrawId=$("#banner_withdrawId").val();
		$.ajax({
	        type: "post", 
	        url: "<%=request.getContextPath()%>/withdraw/update", 
	        dataType: "json", 
	        data: {"approveDesc" : approveDesc,"withdrawId" :withdrawId},
